...
def huber_loss(pred, target, delta=0.2):
    batch_size = pred.size()[0]
    huber_mse = 0.5*torch.sum((target-pred)**2)/batch_size
    huber_mae = torch.sum(delta * (torch.abs(target - pred) - 0.5 * delta))/batch_size
    return torch.where(torch.abs(target - pred) <= delta, huber_mse, huber_mae)
...
#   energy = target[0]
    ...
    #print("True",target,len(target))
    #print(pred.size())
    loss = torch.sum((pred-truth)**2)/batch_size
    #loss = torch.sum((pred-truth)**2)/batch_size
    #loss = torch.sum((pred-torch.log(torch.abs(truth)))**2 +(2.1-pred - torch.log(torch.abs(2.1-truth)))**2)/batch_size
    #loss = torch.sum((pred-truth)**2)/batch_size
    # used for per-sensor energy weighting w/in cluster
#    total_energy_cluster = torch.sqrt(energy*truth)
    # get numer and denom terms for each shower
